...
export default function PaymentPlanSection({ lang }: { lang?: import("@/lib/i18n").LangCode }) {
  const t = useDict();
  return (
    <section id="payment-plan" className="py-20 sm:py-28 bg-[#1A2332] text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-4xl mx-auto">
          <h3 className="font-heading text-xl font-bold text-center mb-6">{t.paymentPlan.tableTitle}</h3>
          <div className="bg-white/5 rounded-xl overflow-hidden">
            <div className="grid grid-cols-3 gap-4 p-4 sm:p-6 bg-white/5 font-semibold text-sm text-[#C8A45C]">
              <span>{t.paymentPlan.tableProject}</span>
              <span>{t.paymentPlan.tableType}</span>
              <span className="text-right">{t.paymentPlan.tablePrice}</span>
            </div>
